...
class LocationInfoViewController: UIViewController {
    ...
    var noLocationImage = UIImageView().then {
        $0.image = UIImage(named: "NoLocation")
    }
    
    var noLocationLabel = UILabel().then {
        $0.text = "위치 정보가 없어요"
        $0.font = Pretendard.regular(19)
        $0.textColor = Gray.medium
    }
    ...
    func setViews() {
        ...
        noLocationView.addSubview(noLocationImage)
        noLocationImage.snp.makeConstraints { make in
        ...
        noLocationLabel.snp.makeConstraints { make in
            make.left.equalTo(noLocationImage.snp.right).offset(4)
    ...
    func locationExist() {
        if NFTResult.latitude == 0 && NFTResult.longitude == 0 {
            locationLabel.isHidden = true
            locationSubLabel.isHidden = true
            map.isHidden = true
            locationDetailButton.isHidden = true
        } else {
